...
def parse_arguments():
    ...
    parser.add_argument('--patricfile', help='List of patric genomes user wants to add into model (Default is list of all patric complete \
                        ...
                        required=False, type=str, default=PATH+'/Database/data/PATRIC_genome_complete_07152018.csv')
    ...
    ###FIGURE OPTIONS###
    parser.add_argument('--figures', help='Generate pathway chemdraw figures', action='store_true')
...
def retrieve_database_info(args):
    ...
    if args.generate_database:
        ...
        database = PATH + '/ConstructedDatabases/' + DEFAULT_DB_NAME + '.db'
        DB = Q.Connector(database)
        ...
        if target_info[0] in incpds_active: #Check if compound exists in organism
            ...
            if optimal_pathways:                    
                ...
                R = rf.ReactionFiles(args.output_path, DB, ex_info.temp_rxns,
                                 target_info[0], target_info[2], incpds_active, args.figures, args.use_iupac_names)
