...
class Output(object):
    ...
    def __init__(self, db, output_path, verbose, RP=False, FBA=False, KO=False, timer_output=False, raw_solutions=True):
        ...
        # if self.RP:
        #     self.ranking_of_pathways = open(output_path+'/ranking_of_solutions_sp.txt', 'w')
        #     self.ranking_of_pathways.write('TargetCpd\tSolution\tRanking Value\tLength of pathway\tPathway Layout and BPs\n')
        #     self.ranking_of_pathways.close()
        if raw_solutions:
            try:
    ...
    # def output_ranking_pathways(self, target, rankingpathways, rankingpathwayslayout, correctly_typed_paths):
    #     with open(self.output_path+'/ranking_of_solutions_sp.txt', 'a') as fout:
    #         for key in rankingpathways:
    #             fout.write("{}\t{}\t{}\t{}\t{}\n".format(target, key, rankingpathways[key], len(correctly_typed_paths[key]), rankingpathwayslayout[key]))
    ...
    def output_raw_solutions(self, compound, org_ID, ordered_paths, reactions, externalrxnscount, incpds, RP):
        ...
        with open(self.output_path+'/raw_compound_solutions/compound_'+str(cpdname)+'_'+str(org_name)+'_outputfile.txt', 'w') as fin:
            # if RP:
            #     for count_pathway, os_dict in reactions.items():
            #         if count_pathway in RP.correctly_typed_paths:
    ...
    def output_shortest_paths(self, target_info, temp_rxns):
        ...
        '''
        def write_to_outputfile(os_dict):
            for r in os_dict:
                if r.endswith('_s'):
    ...
    def output_FBA(self, target_info, org_fbasolution, optimized_fba, comparisonresults, temp):
        ...
        with open(self.output_path+'/flux_individualfluxes_output.txt', 'a') as self.flux_individual_output:
            ...
            for x, value in optimized_fba.fbasol.fluxes.items():
                self.flux_individual_output.write('{}\t{}\n'.format(x, value))
        ...
        with open(self.output_path+'/flux_output.txt', 'a') as self.flux_ouptput:
            ...
            for x, fluxvalue in comparisonresults.fluxchange.items():
                self.flux_ouptput.write('\t{}\t{}\n'.format(x, fluxvalue))
            self.flux_ouptput.write('\nFluxes for added reactions in mutant:'+'\n')
            for r, value in comparisonresults.externalrxnfluxes.items():
                self.flux_ouptput.write('\t{}\t{}\n'.format(r, value))
    ...
    def output_FBA_KOs(self, target_info, fbasolution, compound_dict, comparisonKOresults, temp):
        ...
        with open(self.output_path+'/fluxKO_output.txt', 'a') as self.fluxKO_ouptput:
            ...
            for r, value in comparisonKOresults.fluxchange.items():
        ...
        with open(self.output_path+'/fluxKO_increased_theoreticalyields_output.txt', 'a') as self.fluxKO_in_ty_output:
            if wt_ty != 'NA':
                count = 0
                for rko, value in comparisonKOresults.objective_function_ko.items():
