...
class OptimizeTarget(object):
    ...
    def add_external_metabolites(self):
        ...
        for count, metabolite in enumerate(self.external_metabolites):
            if metabolite in list(self.compounds_dict.keys()):
                pass
            else:
    ...
    def perform_knockouts(self):
        ...
        verbose_print(self.verbose, "STATUS:\tPerforming knockouts")
    ...
    def __init__(self, target_compound_ID, target_organism_ID, model, temp_rxns,
        ...
        for key, os_dict in temp_rxns.items():
            for r in os_dict:
                if r not in self.exrxns:
                    self.exrxns[r] = os_dict[r]
        for key, exmets in temp_exmets.items():
            for c in exmets:
        ...
        if self.exrxns:
            ...
            verbose_print(self.verbose, 'ERROR:\tNo external reactions in solution, likely issue else where in code')
            exit()
