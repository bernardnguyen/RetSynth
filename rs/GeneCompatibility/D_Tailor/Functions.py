...
def structureAnalysis(structure_file, propertyOfInterest="ss"):
    ...
    if path.exists(PATH+"/tmp/structures/"+structure_file+".ct"):
        if propertyOfInterest == "ds":
            output = Popen("perl "+PATH+"/3rdParty/unafold/ss-count.pl "+PATH+"/tmp/structures/" + structure_file + ".ct | awk /[[:digit:]][[:blank:]]0/'{print $1}'", stdout=PIPE, shell=True).stdout.read()
            return [eval(k) for k in output.split()]
        elif propertyOfInterest == "ss":
            output = Popen("perl "+PATH+"/3rdParty/unafold/ss-count.pl "+PATH+"tmp/structures/" + structure_file + ".ct | awk /[[:digit:]][[:blank:]]1/'{print $1}'", stdout=PIPE, shell=True).stdout.read()
            return [eval(k) for k in output.split()]
        else:
            return []
...
def analyze_cai(seq,cai_table=cai_table):
    seq = seq.lower();
    score = 0
    len_sq = 0
    for i in range(0,len(seq),3):
        if seq[i:i+3] in cai_table and cai_table[seq[i:i+3]] > 0:
...
def analyze_terminator(seq):
    ...
    system("echo '>seq1\n" + str(seq) + "' > "+PATH+"/tmp/transterm_files/"+file1)
    system("echo 'fakegene1    1 2    seq1\nfakegene1    " + str(len(seq)-1) + " " + str(len(seq)) +"    seq1' > "+PATH+"/tmp/transterm_files/"+file2)
    #run transtermhp
    output = Popen(PATH+"/3rdParty/transterm/transterm -p "+PATH+"/3rdParty/transterm/expterm.dat "+PATH+"/tmp/transterm_files/"+file1+" "+PATH+"/tmp/transterm_files/"+file2+" 2> "+PATH+"/tmp/transterm_files/err.txt | awk /TERM/'{print $8}'", stdout=PIPE, stderr=None, shell=True).stdout.read()
...
def analyze_duplex_structure(seq1,seq2,filename):
    ...
    call(PATH+"/3rdParty/unafold/hybrid-min -n RNA -o "+filename+" "+ s1_file + " " + s2_file + " -t 37 -T 37", shell = True, stdout = fnull, stderr = fnull)     #code is necessary to omit output generated by UNAFOLD    
...
def analyze_structure(seq,filename,ensemble=False):
    ...
    fnull = open(devnull, 'w')   #this line is necessary to omit output generated by UNAFOLD
    if ensemble:         
        call(PATH+"/3rdParty/unafold/UNAFold.pl -n RNA " + filename + ".seq", shell = True, stdout = fnull, stderr = fnull)     #code is necessary to omit output generated by UNAFOLD
    else:
        call(PATH+"/3rdParty/unafold/hybrid-ss-min -n RNA " + filename + ".seq", shell = True, stdout = fnull, stderr = fnull)     #code is necessary to omit output generated by UNAFOLD
                
    if os.path.isfile(filename+".ct"):        
        system("mv %s*.ct "+PATH+" tmp/structures/" % filename)
        # remove tmp files
...
def SimplePWMScoreOperator(seq, pwmnt, direction=0, mutable_region = None, keep_aa=False, max_iter=100):    
    ...
    motifsize = len(pwmnt[list(pwmnt.keys())[1]])
...
def randomMutationOperator(sequence, keep_aa, mutable_region, cds_region, pos=None, n_mut=[1,2], data_table=cai_table):
    '''
    ...
    if mutableCodonsPosition == [] and mutableUTRPosition == []:
        ...
        if keep_aa == True:
            if (mutableUTRPosition == []) or (mutableCodonsPosition != [] and choice([True,False])):
                return mutateCDS(sequence, keep_aa, mutableCodonsPosition, cds_region, pos, n_mut, data_table)
            else:
...
def mutateCDS(sequence, keep_aa, mutableCodonsPosition, cds_region, pos=None, n_mut=[1,2], data_table=cai_table):
    if keep_aa == True:                
        result = analyzeCodons(sequence,mutableCodonsPosition,data_table=data_table)
...
def SimpleCAIOperator(sequence, cai_range, keep_aa, mutable_region, cds_regions, direction = '+', cai_table=cai_table):
    '''
    ...
    result = analyzeCodons(sequence,mutableCodonsPosition,data_table=cai_table)
    ...
    if mutated == False:
        ...
        all_codons[int(codon_position)]=new_codon
