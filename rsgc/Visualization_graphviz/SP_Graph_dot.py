...
class GraphDot(object):
    ...
    def get_figure(self, cpdID, cpdname, rxn, type_node):
        ...
        if mol:
            if type_node == 'synthetic':
                IN.setOption("render-base-color", "0, 0, 1")
            elif type_node == 'internal':
                IN.setOption("render-base-color", "0.3, 0.3, 0.3")
            else:
                IN.setOption("render-base-color", "1, 0, 0")
    ...
    def sc_graph(self, target_compound_ID, target_organism_ID, paths, images=True, RP=None):
        ...
        self.outputfile_dot.append('    node [fixedsize=true, fontcolor=black, label="", shape=circle, style=filled];\n')
        self.count = 0
        self.store_rxns = set()
        self.nodes = set()
        ...
        if RP:
            for path_count, rxns in paths.items():
