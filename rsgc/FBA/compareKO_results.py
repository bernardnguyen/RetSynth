...
class CompareKO(object):
    ...
    def __init__(self, target_compound_ID, compounds_dict, wtresults, exresults, ex_paths, DB, fold_threshold=1.5):
        ...
        self.ex_rxns = []
        for key, path in self.ex_paths.items():
            for r in path:
    ...
    def get_flux_differences(self):
        ...
        '''
        for rko in tqdm(list(self.exresults.keys())):
            self.fluxchange[rko] = {}
            for r in list(self.exresults[rko].fluxes.keys()):
    ...
    def external_pathanalyze_fluxes(self):
        ...
        for rko in self.fluxchange:
            ...
            if len(self.ex_paths) != 0:
                ...
                self.maxpath[rko] = list(pathwayflux[rko].keys())[list(pathwayflux[rko].values()).index(self.maxflux[rko])]
            else:
