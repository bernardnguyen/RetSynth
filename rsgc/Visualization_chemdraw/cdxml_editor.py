...
class CDXML_Editor(object):
	def __init__(self, cdxml_files_path=None, output_path=None, BP=None, BP_predicted=None, logP=None, logp_predicted=None):
		...
		self.BP = BP
		self.BP_predicted = BP_predicted
		self.logP = logP
		self.logp_predicted = logp_predicted
	...
	def set_output_label(self, text_string, output, color):
		
		label = TEXT(text_string, color=color)
		label.set_x(output.x_reference)
		box = BOX()
		box.append(output)
		box.append(label)
		output = box
	...
	def get_cpd_cdxml(self, compound, id_offset=0, color="5"):
		try:
			...
			if self.BP or self.BP_predicted:
				if compound in self.BP:
					output = self.set_output_label('Boiling point: %.2fC' % self.BP[compound], output, color)

				elif compound in self.BP_predicted:
					output = self.set_output_label('Boiling point (predicted): %.2fC' % self.BP_predicted[compound], output, color)
		
				else:
					output = self.set_output_label('N/A boiling point', output, color)

			elif self.logP:
				if compound in self.logP:
					output = self.set_output_label('LogP: %.2f' % self.logP[compound], output, color)

				elif compound in self.logp_predicted:
					output = self.set_output_label('LogP (predicted): %.2f' % self.logp_predicted[compound], output, color)					

				else:
					output = self.set_output_label('N/A LogP', output, color)
