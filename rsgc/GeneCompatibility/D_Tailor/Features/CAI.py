...
class CAI(Feature):
    ...
    """
    def __init__(self, caiObject = None, solution = None, label="", cai_table=cai_table, 
                        args = { 'cai_range' : (0,59), 
                                 'mutable_region' : None, 
                                 'cds_region' : None, 
                                 'keep_aa' : True }):
    ...
    def set_scores(self, scoring_function=Functions.analyze_cai):     
        self.scores[self.label+"CAI"] = scoring_function(self.sequence, self.cai_table)
                                                                     
    def mutate(self, operator=Functions.SimpleCAIOperator):
        if not self.targetInstructions:
            return None
        new_seq = operator(self.solution.sequence, self.cai_range, self.keep_aa, self.mutable_region, 
                            self.cds_region, self.targetInstructions['direction'],cai_table=self.cai_table)
        if not new_seq:
            return None                         
        return Solution.Solution(sol_id=str(uuid4().int), sequence=new_seq, 
                                cds_region = self.cds_region, mutable_region = self.mutable_region, 
                                parent=self.solution, design=self.solution.designMethod, cai_table=self.cai_table)
