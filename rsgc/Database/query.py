...
class Connector(object):
    ...
    def get_compound_ID(self, compound_name, strict=False):
        ...
        if strict:
            query = "SELECT ID FROM compound WHERE name = '%s' AND compartment <> 'e0'" % compound_name
        else:
    ...
    def get_proteins(self, reaction_ID, organism_ID):
        ...
        query = "SELECT protein_ID from reaction_protein WHERE reaction_ID='%s' and model_ID='%s'" % (reaction_ID, organism_ID)
        conn, cnx = self.connect_to_database()
        Q, cnx = test_db_4_error(conn, cnx, query, self.database, 0)
        hits = fetching_one_query_results(Q, conn, cnx, self.database, query, 0)
        if hits != 'Errored' and hits is not None:
    ...
    def get_reaction_type(self, rxn):
        '''Retrieves reaction on type'''
        if rxn.strip() == "":
            return None

        query = "select type from reaction where ID = '%s'" % rxn
        conn, cnx = self.connect_to_database()
        Q, cnx = test_db_4_error(conn, cnx, query, self.database, 0)
        hits = fetching_one_query_results(Q, conn, cnx, self.database, query, 0)
        if hits != 'Errored' and hits is not None:
    ...
    def get_all_compound_keggIDs(self):
    ...
    def get_all_cpd_with_search(self, search):
        ...
        query = ("SELECT name, ID FROM compound WHERE " +
                        ...
                        " OR chemicalformula LIKE '%" + search + "%'" +
                        " OR ID LIKE '%" + search + "%'" +
                        " OR kegg_id LIKE '%" + search + "%'" +
                        " OR casnumber LIKE '%" + search + "%'" +
                        " AND compartment <> 'e0'"
                    " ORDER BY " +
                        # Exact match
                        "CASE WHEN name = '" + search + "' THEN 2 ELSE 0 END" +
                        " + CASE WHEN ID = '" + search + "' THEN 2 ELSE 0 END" +
                        " + CASE WHEN kegg_id = '" + search + "' THEN 2 ELSE 0 END" +
                        ...
                        " + CASE WHEN kegg_id LIKE '" + search + "%' THEN 1 ELSE 0 END" +
