...
class Generate_database(unittest.TestCase):
    ...
    def test_generate_db(self):
        ...
        query = """select count(*) from sqlite_master where type='table' and name='fba_models'"""
        cnx = sqlite3.connect(PATH+'/test.db')
        Q = cnx.execute(query,)
        hit = Q.fetchone()
        self.assertEqual(hit[0], 1)
        ...
        query = """select count(*) from sqlite_master where type='index' and name='fba_models_ind'"""
        cnx = sqlite3.connect(PATH+'/test.db')
        Q = cnx.execute(query,)
        hit = Q.fetchone()
        self.assertEqual(hit[0], 1)
