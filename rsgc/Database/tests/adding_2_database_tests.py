...
sqlite_database.execute('''CREATE table compound (ID text, name text,compartment text,
                            kegg_id, chemicalformula text, casnumber)''')
...
sqlite_database.execute('''CREATE INDEX fba_models_ind ON fba_models(ID)''')
...
class Adding_to_current_DBTests(unittest.TestCase):
    ...
    def test_adding_to_db(self):
        ...
        bkdb.BuildKbase(PATH+'/data', '../data/KbasetoKEGGCPD.txt', '../data/KbasetoKEGGRXN.txt', False, PATH+'/test.db', 'bio')

        bkdb.BuildKbase(PATH+'/data2', '../data/KbasetoKEGGCPD.txt', '../data/KbasetoKEGGRXN.txt', False, PATH+'/test.db')
