...
bkdb.BuildKbase(PATH+'/data3', PATH+'/KbasetoKEGGCPD_test.txt', PPATH+'/data/KbasetoKEGGRXN.txt',
                True, PATH+'/testinchi.db', 'bio')
conn = sqlite3.connect(PATH+'/testinchi.db')
cnx = conn.cursor()
cnx.execute("UPDATE compound SET kegg_id=? WHERE ID=?", ('cpd00003', 'InChI=1S/C20H15ClF2N2O2/c21-14-6-4-13(5-7-14)12-27-15-8-9-24-19(10-15)25-20(26)11-16-17(22)2-1-3-18(16)23/h1-10H,11-12H2,(H,24,25,26)_c0'))
conn.commit()
...
class RemoveDuplicateCpdsTests(unittest.TestCase):
    ...
    def test_removeduplicatecpds(self):
        ...
        S = cnx.execute('select ID from compound')
        hits = S.fetchall()
        results = [i[0] for i in hits]
        self.assertFalse('cpd00003_c0' in results)

        S = cnx.execute("select cpd_ID from reaction_compound where reaction_ID='%s'"%'rxn8_c0')
        hits = S.fetchall()
        results = [i[0] for i in hits]
        self.assertFalse('cpd00003_c0' in results)
        ...
        S = cnx.execute("select cpd_ID from model_compound where model_ID='%s'" % 't2')
        hits = S.fetchall()
        results = [i[0] for i in hits]
        self.assertFalse('cpd00003_c0' in results)
        self.assertTrue('InChI=1S/C20H15ClF2N2O2/c21-14-6-4-13(5-7-14)12-27-15-8-9-24-19(10-15)25-20(26)11-16-17(22)2-1-3-18(16)23/h1-10H,11-12H2,(H,24,25,26)_c0' in results)
