        ...
        try:
            if m['boundarycondition'] == 'false':
                if inchi:
                    ...
                    modelcompounds_allinfo.append((new_cpdID, m['name'], m['compartment'],
                                                  original_KEGG_cpdID,
                                                  inchi_cf.get(new_cpdID, None),
                                                  inchi_cas.get(new_cpdID, None),
                                                  inchi_pubchem.get(new_cpdID, None)))
                else:
                    modelcompounds.append((new_cpdID, mi))
                    modelcompounds_allinfo.append((new_cpdID, m['name'], m['compartment'], original_KEGG_cpdID, None, None, None))
        except KeyError:
            if inchi:
                modelcompounds.append((new_cpdID, mi))
                modelcompounds_allinfo.append((new_cpdID,
                                               m['name'], m['compartment'], original_KEGG_cpdID,
    ...
    for result in results:
        ...
        if len(results1) == 2:
            cnx.execute("""DELETE FROM reaction_reversibility\
             WHERE reaction_ID = ? AND is_reversible = ?""", (result[0], '0'))
...
def retrieve_exact_inchi_values(m, total_set, inchi_pubchem, inchi_cf, inchi_cas, CPD2KEGG, CT, INCHI):
    ...
    if m['id'] not in total_set:
        ...
        if original_KEGG_ID != None:
            inchi, cas = df.kegg2pubcheminchi(original_KEGG_ID,False)
            if not inchi:
        ...
        if inchi is not None:
            inchi_pubchem[new_KEGG_ID] = inchi
...
def retrieve_metabolic_clusters(DBpath):
    ...
    for key, value in cluster_org.items():
        for v in value:
